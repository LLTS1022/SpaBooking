...
export default function Home() {
  ...
  const [filteredUrl, setFilteredUrl] = useState('https://tsm.spagram.com/api/filter-models.php');
  ...
  const applyFilters = () => {
    ...
    setFilteredUrl(`https://tsm.spagram.com/api/filter-models.php?${queryParams.toString()}`);
  };
  ...
  return (
    <Layout home>
      ...
      <div className={styles.searchHeader}>
        ...
        />
        <button onClick={() => setShowFilters(!showFilters)} className={styles.toggleButton}>
        ...
        <div className={styles.filtersContainer}>
          ...
          >
            {Ethnicities.map((eth, index) => (
              <option
                key={index}
                value={index === 0 ? '' : eth.toLowerCase().replace(/[^a-z]/gi, '')}
              >
                {eth}
              </option>
            ))}
...
{/* <section className={styles.grid}>
  {therapists.map((t, i) => (
    <Link key={i} href={`/therapist/${t.id}`}>
